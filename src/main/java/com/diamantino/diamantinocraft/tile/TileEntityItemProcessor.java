    ...
    protected boolean doWork() {
        ...
        if (!inputs.isEmpty() && canWork(inputs) && hasSpaceForOutputs() && (burner == null || burner.hasFuel())) {
            ...
            if (progress >= workTime) {
                ...
                result = InventoryUtilsComp.mergeItems(this, slotStart, slotStart + getOutputSlotCount(), result);
                if (!result.isEmpty()) {
                    overflow = result;
                }
    ...
    public void tick() {
        ...
        if (!world.isRemote) {
            ...
            if (timer % LONG_UPDATE_DELAY == 0 && !overflow.isEmpty()) {
                ...
                overflow = InventoryUtilsComp.mergeItems(this, start, start + getOutputSlotCount(), overflow);
